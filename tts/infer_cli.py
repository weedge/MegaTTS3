...
class MegaTTS3DiTInfer:
    ...
    def build_model(self, device):
        ...
        if os.path.exists(f"{self.wavvae_exp_name}/model_only_last.ckpt"):
            ...
            load_ckpt(
                self.wavvae, f"{self.wavvae_exp_name}/decoder.ckpt", "model_gen", strict=False
            )
    ...
    def gen(
        ...
        **kwargs,
        ...
        logging.debug(f"| G2P infer: {text} -> {ph_pred} + {tone_pred}")
        logging.info(f"| G2P infer: cost {time.perf_counter()-start_time} s")
        ...
        logging.debug(f"| Duration Prediction infer {text} -> {mel2ph_pred}")
        logging.info(f"| Duration Prediction infer: cost {time.perf_counter()-start_time} s")
        ...
        with torch.cuda.amp.autocast(dtype=self.precision, enabled=True):
            ...
            logging.debug(f"| DIT infer Output: {x.shape}")
            logging.info(f"| DIT infer: cost {time.perf_counter()-start_time} s")

        # WavVAE decode
        x[:, : vae_latent.size(1)] = vae_latent
        start_time = time.perf_counter()
        wav_pred = self.wavvae.decode(x)[0, 0].to(torch.float32)
        logging.debug(f"| WavVAE decode:Output: {wav_pred}")
        logging.info(f"| WavVAE decode: cost {time.perf_counter()-start_time} s")
        ...
        logging.debug(f"| Post-processing Output: {wav_pred}")
        logging.info(
            f"| Post-processing: cost {time.perf_counter()-start_time} s | wav_pred {wav_pred.shape}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--time_step", type=int, default=32, help="Inference steps of Diffusion Transformer"
    )
    ...
    wav_path, input_text, out_path, time_step, p_w, t_w = (
        args.input_wav,
        args.input_text,
        args.output_dir,
        args.time_step,
    ...
    infer_ins = MegaTTS3DiTInfer(ckpt_root=args.ckpt_dir)
    ...
    resource_context = infer_ins.preprocess(
        file_content, latent_file=wav_path.replace(".wav", ".npy")
    )
    ...
    file_path = os.path.join(
        out_path, f"[P]{input_text[:20]}.wav" if args.file_name == "" else args.file_name
    )
